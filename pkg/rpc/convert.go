...
import (
	...
	"github.com/zhanglimao/trivy/rpc/cache"
...
func ConvertToRPCLicense(resources []types.DetectedLicense) []*common.License {
	var rpcResources []*common.License
	for _, r := range resources {
		rpcResources = append(rpcResources, &common.License{
			Severity:   r.Severity,
...
func ConvertFromRPCLicense(resources []*common.License) []types.DetectedLicense {
	var rpcResources []types.DetectedLicense
	for _, r := range resources {
		rpcResources = append(rpcResources, types.DetectedLicense{
			Severity:   r.Severity,
			Category:   ftypes.LicenseCategory(r.Category),
			PkgName:    r.Pkgname,
			FilePath:   r.Filepath,
			Name:       r.Name,
			Link:       r.Link,
...
func ConvertFromRPCResults(rpcResults []*scanner.Result) []types.Result {
	...
	for _, result := range rpcResults {
		results = append(results, types.Result{
			...
			Type:              result.Type,
			Licenses:          ConvertFromRPCLicense(result.License),
...
func ConvertToRPCScanResponse(results types.Results, fos ftypes.OS) *scanner.ScanResponse {
	...
	for _, result := range results {
		rpcResults = append(rpcResults, &scanner.Result{
			...
			License:           ConvertToRPCLicense(result.Licenses),
