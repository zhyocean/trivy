...
func (s Scanner) Scan(ctx context.Context, target, artifactKey string, blobKeys []string, options types.ScanOptions) (types.Results, ftypes.OS, error) {
	...
	if options.Scanners.Enabled(types.LicenseScanner) {
		...
		licenseCategories[ftypes.CategoryForbidden] = licensing.ForbiddenLicenses
		licenseCategories[ftypes.CategoryRestricted] = licensing.RestrictedLicenses
		licenseCategories[ftypes.CategoryReciprocal] = licensing.ReciprocalLicenses
		licenseCategories[ftypes.CategoryNotice] = licensing.NoticeLicenses
		licenseCategories[ftypes.CategoryPermissive] = licensing.PermissiveLicenses
		licenseCategories[ftypes.CategoryUnencumbered] = licensing.UnencumberedLicenses
		licenseResults := s.scanLicenses(artifactDetail, licenseCategories)
