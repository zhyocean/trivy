...
import (
	...
	"github.com/zhyocean/trivy/pkg/report/cyclonedx"
	"github.com/zhyocean/trivy/pkg/report/github"
	"github.com/zhyocean/trivy/pkg/report/predicate"
	"github.com/zhyocean/trivy/pkg/report/spdx"
...
func Write(report types.Report, option Option) error {
	...
	case FormatTemplate:
		...
		if strings.HasPrefix(option.OutputTemplate, "@") && strings.HasSuffix(option.OutputTemplate, "sarif.tpl") {
			log.Logger.Warn("Using `--template sarif.tpl` is deprecated. Please migrate to `--format sarif`. See https://github.com/zhyocean/trivy/discussions/1571")
			writer = &SarifWriter{
				Output:  option.Output,
				Version: option.AppVersion,
			}
